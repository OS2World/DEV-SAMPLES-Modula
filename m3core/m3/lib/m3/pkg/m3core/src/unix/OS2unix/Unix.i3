...
CONST
  ...
  FAPPEND =    16_08;           (* append on each write *)
  FNBLOCK =    16_04;           (* POSIX no delay *)
  FNDELAY =    FNBLOCK;         (* no delay *)
  FSYNCRON =   16_2000;         (* write file syncronously *)
  FASYNC  =    8_020000;        (* Signal pgrp when data ready *)

(******** not implemented on OS2   ******************
...
CONST   (* request *)
  F_DUPFD =  5;   (* Duplicate fd *)
  F_GETFD =  3;   (* Get close-on-exec flag *)
  F_SETFD =  5;   (* Set close-on-exec flag *)
  F_GETFL =  1;   (* Get fd status flags *)
  F_SETFL =  2;   (* Set fd status flags *)
...
CONST (* the tty i/o controls *)
  ...
  TCPIP_FIONREAD =  Or (Or (R, INT), Or (FC, 127));(* Get # bytes to read    *)
  FIONREAD = 16;                              (* emx/include/sys/ioctl.h *)
...
CONST (* flags *)
  ...
  O_TEXT   =    16_10;          (* OS2: text mode *)
  O_BINARY =    16_0100;        (* OS2: binary mode *)
  O_SIZE   =    16_8000;
...
<*EXTERNAL "open" *> PROCEDURE uopen (name: char_star; flags, mode: int): int;
PROCEDURE open (name: char_star; flags, mode: int): int;
