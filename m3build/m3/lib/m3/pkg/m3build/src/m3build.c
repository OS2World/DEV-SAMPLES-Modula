  ...
  char **argv;
  {
    ...
    while (i < argc) {
      if (strcmp (argv [i], "-b") == 0) {
        ...
        printf ("m3build: SRC Modula-3 version 3.5 (February 1, 1995)\n");
        exit (0);
...
static void check_for_makefile ()
  {
    if (readable ("m3makefile")) {
      ...
      local_overrides = "./m3overrides";
    } else if (readable ("../src/m3makefile")) {
      makefile  = dircat (package_dir, "src/m3makefile");
      overrides = dircat (package_dir, "src/m3overrides");
      local_overrides = "../src/m3overrides";
    } else {
